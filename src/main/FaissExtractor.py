...
def generate_case_json(context, api_key):
    prompt = f"""
你是一名资深法律专家，请从以下法律文书中重点、详细、全面地提取案件事实、争议焦点和案由，并输出JSON格式：
1. 案件事实（请分条、详细描述案件经过、各方陈述、证据内容、证据采信与否及理由，内容尽量丰富，**务必准确提取所有涉及的具体数字，如借贷金额、欠款金额、转账金额、赔偿金额等**）
2. 争议焦点（每一项争议焦点都要详细描述，必要时分条列举，说明各方观点及理由，**涉及金额、时间、数量等具体数字要准确提取**）
3. 案由（请详细描述案由的来龙去脉，包括案件起因、各方行为、纠纷经过、争议过程、相关背景等，内容不少于200字，**所有涉及的金额、时间、数量等具体信息要准确提取**）
4. 案件类型（必须属于：劳动纠纷、离婚、民间借贷三类之一）
5. 当事人信息（包括原告和被告，每人包含姓名、性别、民族、生日、住址、身份证号、联系电话、职业、工作单位、代理人等，字段尽量补全，无则填空字符串）
6. 诉讼请求（每一项请求都要详细描述，必要时分条列举，**涉及金额、时间、数量等具体数字要准确提取**）
7. 裁判要点（详细说明裁判理由、法律适用、证据采信与否及理由）
8. 证据清单（原告、被告分别列举所有证据，包含证据名称、内容摘要、来源、采信情况及理由，**证据中涉及的金额、时间、数量等具体数字要准确提取**）
9. 庭审过程摘要（如有，尽量详细提取各方陈述、质证、辩论等环节内容，**涉及金额、时间、数量等具体数字要准确提取**）
10. 其他关键信息（如案件经过、事实认定、法律条文引用等，**所有涉及的具体数字要准确提取**）
...
- 案件事实、争议焦点和案由部分内容要尽量详细、分条、结构化，案由内容不少于200字
- **所有涉及金额、时间、数量等具体数字必须准确提取并在对应字段中体现**
- 当事人信息格式：{{"原告": [{{"姓名": "", "性别": "", "民族": "", "生日": "", "住址": "", "身份证号": "", "联系电话": "", "职业": "", "工作单位": "", "代理人": ""}}, ...], "被告": [{{...}}, ...]}} 
- 证据清单格式：{{"原告": [{{"证据名称": "", "内容摘要": "", "来源": "", "采信情况": "", "采信理由": ""}}, ...], "被告": [{{...}}, ...]}} 
- 其他字段使用数组格式
- 不要包含任何解释性文字
"""
